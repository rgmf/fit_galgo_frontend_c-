...
bool LapsData::load(const rapidjson::Document& document)
{
	...
	if (v.IsObject())
	{
	    ...
	    if (itr_avg_hr != v.MemberEnd() && itr_avg_hr->value.IsFloat())
		lap.avg_heart_rate = itr_avg_hr->value.GetFloat();

	    if (itr_max_hr != v.MemberEnd() && itr_max_hr->value.IsFloat())
		lap.max_heart_rate = itr_max_hr->value.GetFloat();

	    if (itr_min_hr != v.MemberEnd() && itr_min_hr->value.IsFloat())
		lap.min_heart_rate = itr_min_hr->value.GetFloat();

	    if (itr_avg_c != v.MemberEnd() && itr_avg_c->value.IsFloat())
		lap.avg_cadence = itr_avg_c->value.GetFloat();

	    if (itr_max_c != v.MemberEnd() && itr_max_c->value.IsFloat())
		lap.max_cadence = itr_max_c->value.GetFloat();

	    if (itr_avg_rc != v.MemberEnd() && itr_avg_rc->value.IsFloat())
		lap.avg_running_cadence = itr_avg_rc->value.GetFloat();

	    if (itr_max_rc != v.MemberEnd() && itr_max_rc->value.IsFloat())
		lap.max_running_cadence = itr_max_rc->value.GetFloat();

	    if (itr_asc != v.MemberEnd() && itr_asc->value.IsFloat())
		lap.total_ascent = itr_asc->value.GetFloat();

	    if (itr_desc != v.MemberEnd() && itr_desc->value.IsFloat())
		lap.total_descent = itr_desc->value.GetFloat();

	    if (itr_cal != v.MemberEnd() && itr_cal->value.IsFloat())
		lap.total_calories = itr_cal->value.GetFloat();
